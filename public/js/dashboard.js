document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    if (!res.ok)
      throw new Error(user.message || "Erreur récupération utilisateur");
  ...
  function displayUsers(users) {
    ...
    users.forEach((user) => {
      ...
      row.innerHTML = `
                ...
                <td>${user.email}</td>
                <td>${user.role}</td>
                <td><a href="historyUser.html?id=${
                  user._id
                }" class="history-link">Voir</a></td>
  ...
  tableBody.addEventListener("click", async (event) => {
    if (event.target.classList.contains("delete-btn")) {
      ...
      if (confirm("Voulez-vous vraiment supprimer cet utilisateur ?")) {
        try {
          const res = await fetch(`/dashboard/delete_user_by_id/${userId}`, {
            method: "DELETE",
          });
          if (res.ok) {
  ...
  createUserForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (res.ok && result.success) {
        ...
        alert(
          `Erreur : ${result.message || "Impossible de créer l'utilisateur"}`
        );
      }
    } catch (err) {
      console.error(err);
